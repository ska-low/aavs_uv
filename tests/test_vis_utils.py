...
def test_vis_arr_to_matrix_4pol():
    ...
    V = vis_arr_to_matrix_4pol(d, 256, 'lower')
    V = vis_arr_to_matrix_4pol(d, 256, 'lower', conj=False)
    Vc = vis_arr_to_matrix_4pol(d, 256, 'lower', conj=True)
    assert np.allclose(V, np.conj(Vc))
