...
def calc_zenith_apparent_coords(uv: UVX) -> (np.array, np.array, np.array):
    ...
    Args:
        uv (UV): UV data object
    
    Returns:
        app_ras (np.array): Apparent RAs, in radians
        app_decs (np.array): Apparent DECs, in radians
        frame_pos_angle (np.array): Frame position angle, in radians
...
def calc_zenith_tracking_phase_corr(uv: UVX) -> np.array:
    ...
    Returns:
        phs_corr (np.array): Phase corrections to track zenith_at_t0. 
