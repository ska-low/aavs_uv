...
class ApertureArray(object):
    ...
    def __init__(self, uvx: UVX, conjugate_data: bool=False, verbose: bool=False, gsm: str='gsm08'):
        ...
        # Setup all-sky viewer, holography module
        self.asv = AllSkyViewer(self)
    ...
    def set_gsm(self, gsm_str: str='gsm08'):
        ...
        self.gsm       = init_observer(gsm_str)
        self.gsm.lat   = self.uvx.origin.lat.to('rad').value
        self.gsm.lon   = self.uvx.origin.lon.to('rad').value
        self.gsm.elev  = self.uvx.origin.height.to('m').value
    ...
    def get_alt_az(self, sc: SkyCoord) -> SkyCoord:
        ...
        """
        sc.obstime = self._ws('t')
        sc.location = self.earthloc
        return sc.altaz
    ...
    def make_image(self, n_pix: int=128, update: bool=True, vis: str='data') -> np.array:
        ...
        w = self.workspace
        V = self.generate_vis_matrix(vis=vis)
    ...
    def make_healpix(self, n_side: int=128, fov: float=np.pi/2, update: bool=True, apply_mask: bool=True,
    ...
    def generate_gsm(self) -> np.array:
        ...
        import healpy as hp
        import pylab as plt
        sky = self.gsm.generate(self._ws('f').to('MHz').value)
